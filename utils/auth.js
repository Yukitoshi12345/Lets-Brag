...
const withAuth = (req, res, next) => {
  ...
  if (!req.session.loggedIn) {
      ...
      console.log("User attempted to access protected route without authentication.");
      res.redirect('/login');
  } else {
      // User is authenticated, proceed to the protected route
      console.log("User is authenticated and authorized to access the protected route.");
      next();
  }
};
